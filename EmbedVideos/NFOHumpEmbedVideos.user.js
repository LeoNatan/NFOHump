...
(() => {
    ...
    function youtubeShortsEmbedElement(url)
    {
        const exp = /.*youtube\.com\/shorts\/(.*)/;
        const match = url.match(exp);
        return $('<iframe loading="lazy" width="640" height="360" src="https://www.youtube.com/embed/' + match[1] + '" frameborder="0" allowfullscreen="" />');
    }
    ...
    function applyVideoEmbedding()
    {
        ...
        smartFilter('a[href*="youtube.com/watch"').each(function(i, link) {
            //https://m.youtube.com/watch?v=N3nyn_yZQ98
            let replacement = youtubeEmbedElement(link.href);
            applyElementReplacement(link, replacement);
        });
        
        smartFilter('a[href*="youtube.com/shorts"').each(function(i, link) {
            //https://m.youtube.com/watch?v=N3nyn_yZQ98
            let replacement = youtubeShortsEmbedElement(link.href);
            applyElementReplacement(link, replacement);
        });
